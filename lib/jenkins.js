...
Jenkins.prototype.build = function (jobName, params, cb) {
  ...
  function _success(result, cb) {
    ...
    if(!project.hasOwnProperty('scheduledItem') || project.scheduledItem) {
      ...
      cb(new Error('Job ' + jobName + ' was already scheduled'));
    }
  }
  ...
  bag.http.request(method, this.url + '/job/' + jobName + '/' + buildVariant, this.opts, cb);
};
 ...
 *
 * @param {String} jobName: Jenkins job name
 * @param {Integer} number: Jenkins build number
 * @param {Object} opts: optional interval in milliseconds
 * @param {Function} cb: standard cb(err, result) callback
 */
Jenkins.prototype.console = function (jobName, number, opts, cb) {
  var buildNb = 'lastBuild'
  var variant = '';
  ...
  const INTERVAL = 1000;
  var url = this.url + '/job/' + jobName + '/' + buildNb + variant + '/logText/progressiveText',
    self = this;
...
/**
 * Retreive build status.
 *
 * @param {String} jobName: Jenkins job name
 * @param {Integer} buildNb: Build Number
 * @param {Function} cb: standard cb(err, result) callback
 */
 Jenkins.prototype.status = function(jobName, buildNb, cb) {
  ...
  function _success(result, cb) {
    ...
    // Variants
    if(_build.runs) {
      _build.runs.forEach(function(run) {
        var variant = run.url.substr( _build.url.length - _build.number.toString().length + 1);
        variant = variant.substr( 0, variant.indexOf('/') );

        data.variants.push({ name: variant, number: run.number });
      });
    }
  ...
  function _notFound(result, cb) {
    cb(new Error('Build ' + buildNb + ' for job ' + jobName + ' does not exist'));
  }

  this.opts.handlers[200] = _success;
  this.opts.handlers[404] = _notFound;

  bag.http.request('get', this.url + '/job/' + jobName + '/' + buildNb + '/api/json', this.opts, cb);
 };
